...
def run(dataset_dir, net, output_dir, name='MTCNN', shuffling=False):
    ...
    dataset = get_dataset(dataset_dir, net=net, name=name)
...
def get_dataset(dir, net, name):
    #item = 'imglists/PNet/train_%s_raw.txt' % net
    #item = 'imglists/PNet/train_%s_landmark.txt' % net
    item = "%s/%s_%s%s.txt" %(net, name, net, "_aug" if name == "landmark" else "")
    print("getting dataset: ", item)
...
if __name__ == '__main__':
    ...
    run(dir, net, output_directory, "landmark", shuffling=True)
